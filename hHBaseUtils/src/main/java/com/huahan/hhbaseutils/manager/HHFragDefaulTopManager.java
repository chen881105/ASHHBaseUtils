...
public class HHFragDefaulTopManager implements HHTopViewManagerImp
{
	...
	public View getTopView()
	{
		if (mTopView==null)
		{
			mTopView=View.inflate(mFragment.getActivity(), R.layout.hh_include_top_default, null);
	...
	public void setTitleMode(TitleMode mode,int dis)
	{
		...
		if (mode==TitleMode.LEFT)
		{
			...
			layoutParams.leftMargin=HHDensityUtils.dip2px(mFragment.getActivity(), dis);
		}else {
	...
	private void setTopDefaultBackground()
	{
		...
		{
			HHApplication application=(HHApplication) mFragment.getActivity().getApplication();
