...
public class HHCrashHandler implements UncaughtExceptionHandler {
    ...
    * 初始化
    * @param context  
    * @param projectId 项目id（与后台保持一致）
    * @param savePath  bug 信息本地保存路径
    */
    public void init(Context context,String projectId,String savePath) {
        mContext = context;
...
//            file_dir = .getInstance().getFilesDir().getAbsolutePath() + "/crashlog/";
//        }
//        return file_dir;
//    }
    /**
    ...
    private void uploadAppError(final String  error)
    {
        new Thread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                String result=uploadError(error);
            }
        }).start();
    }
    ...
    private   String uploadError(String error)
    {
        ...
        String result = HHWebDataUtils.sendPostRequest_B(requestUrl, map);
        return result;
    }
}
